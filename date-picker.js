(function () {
  // The Start Date field and its Date-select Calendar, shared by both pages:
  // the Start Date editor's field on Badge Overview and the Start Date field
  // in the first Block on Change Dates. Both are the same widget in Journey
  // Tracker, so both are drawn and driven from here.
  //
  //   JourneyDatePicker.init(field, startISO, onChange)
  //
  // field is the [data-controller~="date-picker"] element; onChange is called
  // whenever the field's date changes, and the caller reads the new one from
  // getSelectedISO(). Nothing is written to the Schedule store here: what a
  // change means is the caller's business.
  //
  // Some static exports of these pages prune CSS rules for widgets that
  // weren't open at capture time, leaving classes like .cal-popup-grid without
  // their styles. Re-declaring them is harmless when the rules already exist
  // (identical values) and required when they don't.
  ...
  function initDatePicker(field, startISO, onChange) {
  ...
  window.JourneyDatePicker = { init: initDatePicker };
})();
